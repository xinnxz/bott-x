...
class Tomarket {
    ...
    async login(data) {
        ...
        try {
            ...
            const isAccessToken = res?.data?.data?.access_token
            
            if (!isAccessToken) {
                this.log(colors.red(`Login không thành công! Mã trạng thái: ${res.data.message}`));
                return null;
            }
	...
	async processTasks(tasks, type, initData, maxRetries = 5) {
        const taskExcludes = [213]
		const tasksToProcess = tasks.filter(task => type === 'default' ? task.status === 0 : true).filter((e) => !taskExcludes.includes(e?.taskId));
