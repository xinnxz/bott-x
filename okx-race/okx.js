...
class OKX {
    ...
    async main() {
        ...
        let hoinangcap = nangcapfueltank.toLowerCase() === 'y';
        const nangcapturbo = await this.askQuestion('Bạn có muốn nâng cấp Turbo Charger không? (y/n): ');
        let hoiturbo = nangcapturbo.toLowerCase() === 'y';

        while (true) {
            for (let i = 0; i < userData.length; i++) {
                const queryId = userData[i];
                const { extUserId, extUserName, queryDecode } = this.extractUserData(queryId);
                this.log(`===== Bắt đầu tài khoản ${extUserName} =====`.bgYellow.red);
                try {
                    await this.checkDailyRewards(extUserId, queryDecode);

                    let boosts = await this.getBoosts(queryDecode);
                    ...
                    if (fuelTank && hoinangcap) {
                        const balanceResponse = await this.postToOKXAPI(extUserId, extUserName, queryDecode);
                        ...
                        if (fuelTank.curStage < fuelTank.totalStage && balancePoints > fuelTank.pointCost) {
                            await this.upgradeFuelTank(queryDecode);
                            
                            boosts = await this.getBoosts(queryDecode);
                            const updatedFuelTank = boosts.find(boost => boost.id === 2);
                            const updatebalanceResponse = await this.postToOKXAPI(extUserId, extUserName, queryDecode);
                    ...
                    if (turbo && hoiturbo) {
                        ...
                        if (turbo.curStage < turbo.totalStage && balancePoints > turbo.pointCost) {
                            await this.upgradeTurbo(queryDecode);
                    ...
                    for (let j = 0; j < 10; j++) {
                        const response = await this.postToOKXAPI(extUserId, extUserName, queryDecode);
                        const balancePoints = response.data.data.balancePoints;
                        this.log(`${'Balance Points:'.green} ${balancePoints}`);
    
                        const predict = 1;
                        const assessResponse = await this.assessPrediction(extUserId, predict, queryDecode);
                        ...
                        if (assessData.numChance === j && reloadFuelTank && reloadFuelTank.curStage < reloadFuelTank.totalStage) {
                            await this.useBoost(queryDecode);
                            boosts = await this.getBoosts(queryDecode);
                            ...
                            await this.Countdown((28 - assessData.numChance) * 90);
                        }
                    }
